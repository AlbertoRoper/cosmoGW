"""
interferometry_plots.py is a Python routine that contains functions relevant
to generate the plots of the interferometry Jupyter notebook where the
interferometry using space-based GW detectors, like LISA or Taiji is
studied, following appendix B of A. Roper Pol, S. Mandal, A. Brandenburg, and
T. Kahniashvili, "Polarization of gravitational waves from helical MHD turbulent sources",
JCAP 04 (2022) 019, https://arxiv.org/abs/2107.05356.

It is used in the tutorials response_functions.ipynb and interferometry.ipnyb
"""
...
def plot_Mf(save=True, ED_I=False):
    ...
    fs, MAs, MTs, DAEs = inte.read_response_LISA_Taiji(TDI=True, interf='LISA')
    fs, MAs_Tai, MTs_Tai, DAEs_Tai = inte.read_response_LISA_Taiji(TDI=True, interf='Taiji')
    fs, MAs_Tai, MTs_Tai, DAEs_Tai = inte.read_response_LISA_Taiji(TDI=True, interf='Taiji')
    
    # read correlated ED response
    f_ED_I, M_ED_I = inte.read_MAC(M='MED', V='I')
    ...
    if ED_I == True:
        ...
        aaux = 'ED'
    Rf = inte.R_f(fs)
    Rf_Tai = inte.R_f(fs, L=inte.L_Taiji)
...
def plot_Df(save=True):
    ...
    plt.text(1.2e-2, 7e-3, r'${\cal D}_{CD}(f)$', fontsize=30)
...
def DAE_posneg(f, D, lw=1, ls='solid'):
    ...
    of a function D, used in 'plot_Df'
...
def plot_noise_PSD(f=inte.f_ref, interf='LISA', save=True):
    ...
    ### The power spectral density of the noise can be computed using interferometry.py
    PnA, _ = inte.Pn_f(f, TDI=True)
    PnC, _ = inte.Pn_f(f, P=inte.P_Taiji, A=inte.A_Taiji, L=inte.L_Taiji, TDI=True)
    
    # and the specific noise functions
    Pacc = inte.Pacc_f(f, A=inte.A_LISA, L=inte.L_LISA)
    Pacc_Tai = inte.Pacc_f(f, A=inte.A_Taiji, L=inte.L_Taiji)
    Poms = inte.Poms_f(f, P=inte.P_LISA, L=inte.L_LISA)
    Poms_Tai = inte.Poms_f(f, P=inte.P_Taiji, L=inte.L_Taiji)

    plt.figure(figsize=(12,8))
    if interf=='LISA':
        plt.plot(f, PnA, color='red')
        plt.plot(f, Pacc, color='blue', ls='-.', lw=.8)
        plt.plot(f, Poms, color='blue', ls='-.', lw=.8)
        yPn = 1e-40
        yPoms = 1e-40
        yPacc = 6e-43
        A = 'A'
    ...
    if interf=='Taiji':
        plt.plot(f, PnC, color='red')
        plt.plot(f, Pacc_Tai, color='blue', ls='-.', lw=.8)
        plt.plot(f, Poms_Tai, color='blue', ls='-.', lw=.8)
        yPn = 2e-41
        yPoms = 2e-41
        yPacc = 4e-43
        A = 'C'
    ...
    plt.xlabel('$f$ [Hz]')
    plt.ylabel('noise PSD $P_n (f)$ [Hz$^{-1}$]')
    plt.xscale('log')
    plt.yscale('log')
    plt.ylim(1e-44, 1e-30)
    plt.xlim(1e-5, 1e0)
...
def plot_sensitivity(save=True):
    ...
    # Sensitivity of the cross-correlated channel of the LISA-Taiji network
    # Stokes parameter I, Sn_ED_I
    fs_ED, Sn_ED_I, _ = inte.Sn_f(interf='comb', TDI=True, M='MED')

    plt.figure(figsize=(12,8))
    plt.rc('font', size=20)
    plt.plot(fsX, np.sqrt(SnX), color='red', label='channel X')
    plt.plot(fs, np.sqrt(SnA), color='blue', label='TDI channel A')
    plt.plot(fs, np.sqrt(SnT), color='orange', label='TDI channel T',
             alpha=.6)

    plt.plot(fsX_Tai, np.sqrt(SnX_Tai), color='red', ls='-.')
    plt.plot(fs_Tai, np.sqrt(SnC), color='blue', ls='-.')
    plt.plot(fs_Tai, np.sqrt(SnS), color='orange', ls='-.', alpha=.6)

    plt.plot(fs_ED, np.sqrt(abs(Sn_ED_I)), color='purple', alpha=.5,
             label='TDI channels ED')

    plt.legend(fontsize=20)
    plt.xlabel('$f$ [Hz]')
    plt.ylabel(r'Strain sensitivity $\sqrt{S_h(f)}$ $\left[{\rm Hz}^{-1/2}\right]$')
    plt.xscale('log')
    plt.yscale('log')
    plt.text(6e-2, 1e-19, 'LISA', fontsize=20)
...
def plot_Omega_sensitivity(SNR=inte.SNR_PLS, T=inte.T_PLS, save=True, h0=1.):
    ...
    fs, OmSA, OmST = inte.compute_Oms_LISA_Taiji(interf='LISA', TDI=True, h0=h0)
    fs, OmSC, OmSS = inte.compute_Oms_LISA_Taiji(interf='Taiji', TDI=True, h0=h0)
    
    ## Sensitivities of LISA and Taiji to compute the combined sensitivity
    OmS_comb = 1/np.sqrt(1/OmSA**2 + 1/OmSC**2)
    
    # Sensitivity of cross-correlating E (LISA) and D (Taiji) TDI channels
    fs_ED, Sn_ED, _ = inte.Sn_f(interf='comb', TDI=True, M='MED', Xi=False)
    OmS_ED = inte.Oms(fs_ED, abs(Sn_ED), h0=h0)
    
    # PLS sensitivity
    OmPLS_A = inte.OmPLS(OmSA, f=fs, SNR=SNR, T=T)
    OmPLS_C = inte.OmPLS(OmSC, f=fs, SNR=SNR, T=T)
    OmPLS_comb = inte.OmPLS(OmS_comb, f=fs, SNR=SNR, T=T)
    ...
    plt.plot(fs, OmPLS_A, color='green')
    plt.plot(fs, OmSC, '-.', color='blue')
    plt.plot(fs, OmSS, '-.', color='orange', alpha=.5)
    plt.plot(fs, OmPLS_C, '-.', color='green')
    plt.plot(fs_ED, OmS_ED, color='purple', alpha=.5)
    plt.plot(fs, OmS_comb, color='blue', lw=.8)
    plt.plot(fs, OmPLS_comb, color='green', lw=.8)
